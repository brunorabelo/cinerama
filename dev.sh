...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=cinerama.example.com
...
workon cinerama  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development cinerama npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop cinerama
    docker rm cinerama
    docker run --name cinerama -d --env-file /home/ubuntu/cinerama.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/cinerama:/dkdata \
        cinerama start_web.sh
}
...
echo_green "Welcome to cinerama's dev env"
